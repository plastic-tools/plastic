...
import { $PipeInput, Channel, nothing } from "../core";
...
interface QueueWriter<T> {
  ...
  /**
   * Closes the channel if it's not already closed. If a value is passed,
   * it will be emitted first.
   */
  close(value?: T): void;
...
class QueueChannel<T> implements Channel<T>, QueueWriter<T> {
  ...
  close(value?: T) {
    if (value !== undefined) this.put(value);
